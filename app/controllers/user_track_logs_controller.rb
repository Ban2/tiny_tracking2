class UserTrackLogsController < ApplicationController
  ...
  def index
    @user_track_logs = UserTrackLog.paginate(:page => params[:page], :per_page => 30)
  ...
  def create
    ...
    user_name = params[:user_track_log][:user_name]
    @user = User.find_by_name(user_name)
    if  @user.blank?
     redirect_to root_path, notice: 'Invalid User Name' 
     return
   end
    @user_track_log = UserTrackLog.new(user_track_log_params)
    @user_track_log.arrival_time = Time.now
    @user_track_log.user_name = user_name
    @user_track_log.user_id =   @user.id
    respond_to do |format|
      if @user_track_log.save
        #format.html { redirect_to @user_track_log, notice: 'You have Signed in successfully.' }
